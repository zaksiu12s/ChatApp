...
export const setupSocket = (server: any) => {
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('sendMessage', async ({ value, senderUsername, token }) => {
            if (!value || !senderUsername || !token || typeof value !== 'string' || typeof senderUsername !== 'string' || typeof token !== "string") {
                io.to(socket.id).emit('error', { message: "Invalid username or message", status: 400 });
                return
            };

            try {
                const tokenUsername = await verifyToken(token, SECRET_KEY);
                if (tokenUsername !== senderUsername) {
                    io.to(socket.id).emit('error', { message: "Token does not match sender's username", status: 401 });
                    return;
                }
            } catch (err) {
                io.to(socket.id).emit('error', { message: "Token validation failed", status: 401 });
